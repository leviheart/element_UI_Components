export function relationHidden(params = {}) {
  ...
  // 是否存在被控制的对象
  const relation_hidden =
    hidden_obj && Array.isArray(hidden_obj) && hidden_obj.length > 0;
  if (!relation_hidden) {
    return false;
  }
  // 处理交互
...
export function relationDisabled(params = {}) {
  ...
  // 是否存在被控制的对象
  const relation_disabled =
    disabled_obj && Array.isArray(disabled_obj) && disabled_obj.length > 0;
